...
#Justification of choice of tools
#Reproducible: accession number, software packages
#Code --> attachment/ github repository
#citations include packages

#GSE50697: series of interest
...
#Reading CEL file using affybatch for normalisation and analysis
data <- read.affybatch(paste0('data/',list.celfiles('data')))
methods=class(class="data") #to see a list of functions we can use on this data file

#Pseudo-image of the chip
image(data[,-1])
...
sapply(GSMList(gse),treatment_type) #uses the new function made in the previous step to append treaatment metadata in the list of GSMs acquired from the GSE
...
celfiles <- paste0('data/',rownames(pd),'.CEL') #since the row names of the dataframe is e.g. GSM1226581 
...
contrasts
#Naturally, you can perform more complex analysis with multiple comparisons 
#depending on the question of interest. You can read up more on using limma 
#for more complex analysis from the limma user guide 
#(available at https://bioconductor.org/packages/release/bioc/vignettes/limma/inst/doc/usersguide.pdf, 
#and in particular, pages 35-64), which demonstrates the use of limma for a 
#wide range of questions and even two-colored platforms.

#One such confounder frequently encountered in microarrays is batch effect
#exploratory data analysis (EDA) is critical to understanding the nature of 
#data prior to model fitting. While outside the scope of this practical, 
#it is a worthwhile investment to find out some of these methods and 
#also how one can correct for these technical differences in a 
...
#By default, topTable() returns only the top ten genes ranked by their B value. 
#How do you get the statistics associated with all the genes on the array? 
#Explore the options of topTable to identify lists of genes that fit your criteria of interest.
...
#at least two (log fold change at least one).
...
#Volcano plot
interesting_genes <- topTable(fitted.ebayes,number=Inf,p.value = 0.05,lfc=1.5)
interesting_genes
...
print(interesting_genes_annotated[3], row.names = FALSE) #copy and paste the results here into David webpage
